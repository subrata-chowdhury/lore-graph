...
const Title: React.FC<Props> = ({
  ...
  return (
    <div className="group/title relative flex flex-col items-center">
      <div
        className={
          "absolute -top-2 z-10 hidden w-fit -translate-y-full rounded bg-gray-200 p-2 px-3 text-xs text-nowrap select-none group-hover/title:block group-active/title:block " +
          className
        }
      >
        {title}
      ...
      <div className={"cursor-pointer select-none " + containerClassName} onClick={onClick}>
        {children}
      </div>
    </div>
  );
