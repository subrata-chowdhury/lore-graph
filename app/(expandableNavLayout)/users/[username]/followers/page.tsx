...
const FollowersPage = () => {
  ...
  const [data, setData] = useState<FollowerType[]>([]);
  ...
  const fetchFollowers = useCallback(
    async ({ search }: { search: string }) => {
      ...
      }>(`/users/${username}/followers?${queryParams.toString()}`);

      if (res.status === 200 && res.body) {
        setData(res.body.followers);
  ...
  const debouncedSearch = useMemo(() => debounce(fetchFollowers, 500), []);
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-3 p-4 sm:gap-6">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
        <h1 className="text-xl font-bold sm:text-2xl">Followers</h1>
          ...
          {data.map((user) => (
            <UserCard
              ...
              user={{
                ...
                profileImage: user.followerProfileImage,
              }}
            />
          ))}
        </div>
