...
export async function POST(req: NextRequest) {
  ...
  try {
    async function findByEmail(email: string) {
      const collections = [SuperAdmin];
      const promises = collections.map((collection) =>
        (collection as typeof SuperAdmin)
          .findOne({ email })
          .exec()
          .then((result: InstanceType<typeof SuperAdmin> | null) => ({
    ...
    const token = await new SignJWT({
      ...
      .sign(new TextEncoder().encode(process.env.SUPER_ADMIN_JWT_SECRET));
    ...
    chunks.forEach((chunk, index) => {
      res.cookies.set(`super_admin_session.${index}`, chunk, {
        sameSite: "strict",
        path: "/",
        maxAge: 8 * 60 * 60,
      });
    ...
    res.cookies.set("super_admin_session_chunks", chunks.length.toString(), {
      sameSite: "strict",
      path: "/",
      maxAge: 8 * 60 * 60,
    });
