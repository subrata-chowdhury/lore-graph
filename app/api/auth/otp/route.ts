...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    async function findById(id: string) {
      ...
      const promises = collections.map((collection) =>
        collection
          .findById(id)
          .exec()
          .then((result) => ({ model: collection.modelName, data: result }))
      );
    ...
    user.verified = true;
    user = {
      ...(await User.findByIdAndUpdate(id, { otp: null, verified: true }, { new: true })),
      type: user.type,
    };
    ...
    return NextResponse.json(
      {
        ...
        user: { type: user.type, institution: user.institution || "" },
      },
      { status: 200 }
    );
  } catch (error) {
